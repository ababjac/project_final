...
pygame.mixer.set_num_channels(384) #resize to have enough channels for every sound
...
class sample(object):
    ...
    #play the sound
    def play_sound(self):
        #use channels so multiple sounds can be played at once
        channel = pygame.mixer.find_channel()
        channel.play(self.file)
...
sample_dict = {}
for i in range(0,32):
    sample_dict['button%s' %(1+(i*12)-1)] = sample('../drum_sounds/Bass-Drum-1.wav')
    sample_dict['button%s' %(2+(i*12)-1)] = sample('../drum_sounds/Hip-Hop-Snare-1.wav')
    sample_dict['button%s' %(3+(i*12)-1)] = sample('../drum_sounds/Electro-Tom.wav')
    sample_dict['button%s' %(4+(i*12)-1)] = sample('../drum_sounds/Ensoniq-ESQ-1-Hi-Synth-Tom.wav')
    sample_dict['button%s' %(5+(i*12)-1)] = sample('../drum_sounds/E-Mu-Proteus-FX-Wacky-Crash-Cymbal.wav')
    sample_dict['button%s' %(6+(i*12)-1)] = sample('../drum_sounds/Clap-1.wav')
    sample_dict['button%s' %(7+(i*12)-1)] = sample('../drum_sounds/Korg-M1-Open-Hi-Hat.wav')
    sample_dict['button%s' %(8+(i*12)-1)] = sample('../drum_sounds/Korg-NS5R-Power-Closed-Hi-Hat.wav')
    sample_dict['button%s' %(9+(i*12)-1)] = sample('../drum_sounds/Korg-N1R-Pedal-Hi-Hat.wav')
    sample_dict['button%s' %(10+(i*12)-1)] = sample('../drum_sounds/Bass-Drum-2.wav')
    sample_dict['button%s' %(11+(i*12)-1)] = sample('../drum_sounds/Dry-Kick.wav')
    sample_dict['button%s' %(12+(i*12)-1)] = sample('../drum_sounds/Deep-Kick.wav')
